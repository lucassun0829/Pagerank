...
def pagerank(digraph, num_iterations=40, damping_factor=.85):
    ...
    for node in digraph.nodes():
        res[node.identifier()] = 1/length
        temp_dict[node.identifier()] = []
        sink_nodes[node.identifier()] = True
    const = (1-damping_factor)/length
    for tempe in digraph.edges():
        node1 = tempe.nodes()[0].identifier()
        temp_dict[tempe.nodes()[1].identifier()].append(node1)
        if node1 in sink_nodes:
            del sink_nodes[node1]
    while num_iterations > 0:
        for pair in res.items():
            sum1 = 0
            sink = 0
            for node in temp_dict[pair[0]]:
                sum1 += res[node] / digraph.out_degree(node)
            for node in sink_nodes:
                sink += res[node] / length

            next_res[pair[0]] = const + damping_factor*(sum1 + sink)
        res = next_res.copy()
        num_iterations -= 1
    return res
